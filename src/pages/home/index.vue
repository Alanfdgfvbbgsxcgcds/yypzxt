<template>
  ...
  <van-swipe class="my-swipe" :autoplay="3000" height="170" indicator-color="white">
    <van-swipe-item v-for="item in indexList.slides" :key="item.id">
      <van-image :src="item.pic_image_url" radius='5' />
    </van-swipe-item>
  </van-swipe>

  <van-row justify="space-around" style=" margin-top:5px">
    <van-col span="11" v-for="(item,index) in  indexList.nav2s" :key="item.id" @click="goOrderTwo(index)">
      <van-image :src="item.pic_image_url" />
    </van-col>
  </van-row>

  <van-row class="yy-list" @click="getOrder(item)" v-for="(item,index) in indexList.hospitals" :key="item.id"
...
<script setup>
  ...
  import { layoutIndex } from '../../api/index.js'
  ...
  onMounted(() => {
    layoutIndex().then(({ data: { data } }) => {
      // console.log(data);
      indexList.navs = data.navs
      indexList.hospitals = data.hospitals
      indexList.nav2s = data.nav2s
      indexList.slides = data.slides
    })
  })
  // 快捷入口
  const goOrderTwo = (index) => {
    router.push(`/createOrder?id=${indexList.hospitals[index].id}`)
  }
